                    ...
                    if let Some(route) = &self.current_route {
                        match route {
                            ...
                            AppRoute::Register => html!{
                                <Register callback=callback_register />
                            },
                            AppRoute::Home => html!{
                                <Home />
                            },
                            AppRoute::Editor(slug) => html!{
                                <Editor slug=Some(slug.clone()) />
                            },
                            AppRoute::EditorCreate => html!{
                                <Editor />
                            },
                            AppRoute::Article(slug) => html!{
                                <Article slug=slug.clone()
                                         current_user=self.current_user.clone() />
                            },
                            AppRoute::Settings => html!{
                                <Settings callback=callback_logout />
                            },
                            AppRoute::ProfileFavorites(username) => html!{
                                <Profile username=username.clone()
                                         current_user=self.current_user.clone()
                                         tab=ProfileTab::FavoritedBy
                                />
                            },
                            AppRoute::Profile(username) => html!{
                                <Profile username=username.clone()
                                         current_user=self.current_user.clone()
                                         tab=ProfileTab::ByAuthor
                                />
                            },
                        }
                    } else {

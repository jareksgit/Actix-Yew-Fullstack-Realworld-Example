...
impl MainView {
    fn your_feed_tab(&self) -> Html {
        if is_authenticated() {
            let (msg, class) = self.clone().get_tab_msg_class(Tab::Feed);
    ...
    fn global_feed_tab(&self) -> Html {
        let (msg, class) = self.clone().get_tab_msg_class(Tab::All);
        let onclick = self.link.callback(move |ev: MouseEvent| {
            ev.prevent_default();
            msg.clone()
        });
    ...
    fn tag_filter_tab(&self) -> Html {
        if let Some(tag) = &self.props.tag {
            let (msg, class) = self.clone().get_tab_msg_class(Tab::Tag);
            let onclick = self.link.callback(move |ev: MouseEvent| {
                ev.prevent_default();
                msg.clone()
            });
